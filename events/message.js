...
client.on("message", async (message) => {
  ...
  boos.findOne({ guild: message.guild.id }, async (err, data) => {
    if (message.type === "USER_PREMIUM_GUILD_SUBSCRIPTION") {
      ...
      const boostembeed = new MessageEmbed()
        .setTitle(`${message.author} ha mejorado el servidor!`)
        .setDescription(`Gracias <@${message.author.id}> por mejorar el servidor, reclama tus recompensas en <#944014046716366871>`)
        .addFields(
          { name: 'Beneficios del boost', value: '✅・100 Robux con Tax, en cuanto boostees la Owner del servidor te mandara solicitud para pagarte.\n\n✅・Habra dupeos para los Booster.\n\n✅・Tendras un rango exclusivo y perms para mandar fotos y links.\n\n✅・Te regalaremos 500k de DHC en cuanto boostees.' },)
        

      ch.send(boostembeed)/*`Thank you ${message.author}, for becoming a Nitro Booster! You have unlocked:\n\n
